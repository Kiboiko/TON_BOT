...
Перед первой сборкой поправьте `public/tonconnect-manifest.json`: поля `url`
и `iconUrl` должны указывать на ваш домен. В репозитории там плейсхолдер
`example.com` — с чужим доменом кошельки откажутся подключаться.
