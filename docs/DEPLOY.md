...
### TON-сайт: прямая отдача домена (`ton-site`)

Публичные шлюзы (`ton.website`, `ton.run`) отдают только TON Storage и делают
это ненадёжно, а `ton.limo` вообще перестал существовать. Поэтому платформа
поднимает собственный вход в сеть TON — `rldp-http-proxy` в режиме сервера.
Домен с DNS-записью `site` открывается напрямую с нашего сервера.

```bash
docker compose --profile storage up -d ton-site
docker logs ton-site-builder-ton-site-1 | grep TON_SITE_ADNL
```

Полученный адрес (55 символов) пропишите в `backend/.env` и перезапустите
backend:

```
TON_SITE_ADNL=<адрес из лога>
TON_SITE_ADNL_ADDR=<публичный_ip_сервера>:3335
```

Ключ создаётся один раз и лежит в томе `ton_site_data`. **Его нельзя терять и
нельзя пересоздавать:** адрес записан в DNS доменов пользователей, и после
смены ключа каждый владелец домена должен будет заново подписывать запись.
Порт 3335/udp открывается наружу.
...
домена. Запись `storage` при этом остаётся: одна не мешает другой.
