...
Сам проект создаётся свободно, а возможность загружать свой код оплачивается
**разово и отдельно на каждый сайт** — это не подписка. Цена лежит в строке
тарифа `kind: "custom_code"` и меняется в админке; неоплаченный код не попадает
ни в превью, ни в публикацию.

| Метод | Путь | Ответ |
|---|---|---|
| GET | `/api/sites/{id}/custom-code/price` | `{ price_ton, paid }` |
| POST | `/api/sites/{id}/custom-code/purchase` | `{ transaction, payment_id }` |
| POST | `/api/sites/{id}/custom-code/confirm` | `{ success }` — после проверки платежа on-chain |
| POST | `/api/sites/{id}/custom-code` | `{ success }` |

Ошибки: `402 CUSTOM_CODE_NOT_PAID` — не оплачено; `409 CUSTOM_CODE_PAID` — уже
оплачено; `400 NOT_A_CUSTOM_CODE_SITE` — сайт другого типа;
`400 CUSTOM_CODE_TOO_LARGE` — превышен лимит размера.

Публикация — отдельная история: она по-прежнему требует подписки или бесплатного
пробного периода, как и у сайтов остальных типов.
