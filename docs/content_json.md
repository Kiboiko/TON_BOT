...
```jsonc
{
  ...
  "theme": { "preset": "aurora", "accent": "#0098ea", "background": "",
             "background_image": "", "background_dim": 0 },
  "blocks": [
...
| `background_image` | ссылка на картинку, обычно `/u/…` из `POST /api/uploads` | фотография во весь экран; ложится слоем поверх `background`/пресета, который остаётся запасным цветом |
| `background_dim` | целое 0–90 | затемнение фотографии в процентах, слой поверх неё; без него светлый текст на светлом снимке не читается |

Фон собирается в один CSS-шорткат, слои сверху вниз: затемнение → фотография
(`center / cover no-repeat`) → `background` или пресет. Ссылка на фото проходит
ту же проверку схемы, что и остальные ссылки, и дополнительно отбрасывается,
если содержит кавычки, скобки или пробелы — иначе она закрыла бы `url(...)`
и позволила дописать произвольный CSS.
...
Это отдельный тип сайта (`type: "custom_code"`), а не блок внутри другого
проекта: у него пустой список блоков, а страница целиком собирается из кода
пользователя. Код хранится **отдельно** от `content_json` — в поле `custom_code`:

```json
{ "html": "<div class='promo'>…</div>", "css": ".promo{…}", "js": "console.log(1)" }
```

* Загружается через `POST /api/sites/{id}/custom-code` и только при активной
  подписке, иначе `402 SUBSCRIPTION_REQUIRED`.
* Суммарный лимит — `MAX_CUSTOM_CODE_BYTES` (256 КБ), иначе `400 CUSTOM_CODE_TOO_LARGE`.
* В опубликованном сайте и в превью рендерится в изолированном iframe.
